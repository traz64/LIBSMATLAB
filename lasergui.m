...
% Last Modified by GUIDE v2.5 19-Nov-2015 16:17:43
...
calibrationset_data = get(handles.calibrationsetEdit, 'String');
plsdata = get(handles.plsmodelEdit, 'String');

    switch newMode
        case 'train';
            if(isempty(calibrationset_data))
                msgbox('Please select a calibration set.');
            else
                rock_data = calibrationset_data;
                close;
                laseranalysis(rock_data, plsdata, ncomp, newMode, custom_thresh);
            end
        case 'test';
            if(isempty('handles.txt2mdir'))
                msgbox('Please select a testing data folder.');
            elseif(isempty(plsdata))
                msgbox('Please select a PLS model.');
            else
                rock_data = text_to_matlab(handles.txt2mdir,1);
                close;
                laseranalysis(rock_data, plsdata, ncomp, newMode, custom_thresh);
            end
        case 'testset';
            if(isempty('handles.txt2mdir'))
                msgbox('Please select a testing set folder.');
            elseif(isempty(plsdata))
                msgbox('Please select a PLS model.');
            else
                rock_data = text_to_matlab(handles.txt2mdir,2);
                close;
                laseranalysis(rock_data, plsdata, ncomp, newMode, custom_thresh);
            end
...
try
    ResLoc = check_create_dir('Results',mfilename('fullpath'),2);
    PLSmLoc = check_create_dir('PLS Models',mfilename('fullpath'),2);
    size = round((directory_size(ResLoc) + directory_size(PLSmLoc))/1048576,1);
    if(size >= 100)
        size = num2str(size);
        warndlg(['The current total size of the "Results" and "PLS Models" directories is ' size 'MB. Consider archeiving or deleting old Testing and Training results to prevent too much disk space from being used'],'Warning!')
    end
catch
    warndlg('Current size of the "Results" and "PLS Models directories could not be determined. Make sure to archeive or delete old Testing and Training results, and PLS models periodically to prevent too much disk space from being used','Warning!')
end
...
if (strcmp(newMode, 'trainingButton'))
    BadCal = 0;
    Xloc = uigetfile('.', 'Select the calibration set .mat file to be used for PLS model creation.');
    if (Xloc == 0)
        clearvars Xloc
    else
        ...
        if(BadCal ~= 1)
            if (exist('rock_data','var'))     %Checks if .mat file is Calibration data
                clearvars rock_data
                set(handles.calibrationsetEdit,'String',Xloc)
...
if (strcmp(newMode, 'testingButton'))
    ...
    if(singlefolder == 0)
        ...
        if(BadTst ~= 1)
            if(~isempty(strfind(TScheck,'''Start Wavelength')))
                PLSloc = uigetfile('.', 'Select the PLS model .mat file to use.');     
                if (PLSloc == 0)
                    ...
                    if(BadPLS ~= 1)
                        if (exist('PLS_Model_All','var'))     %Checks if .mat file is Calibration data
                            ...
                            set(handles.plsmodelEdit,'String',PLSloc)
...
set(helpBox, 'position', [550 150 330 495])
